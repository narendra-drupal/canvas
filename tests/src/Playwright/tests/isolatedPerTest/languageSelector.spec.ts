...
test.describe('Language Selector', () => {
  ...
  test.beforeEach(async ({ drupal }) => {
    ...
    const addLanguageResult = await execDrush('language:add fr', {
      url: drupalSite.url,
      userAgent: drupalSite.userAgent,
    });
    console.log('Add French language result:', addLanguageResult);

    // Verify French was added successfully
    const languageListResult = await execDrush('language:info', {
      url: drupalSite.url,
      userAgent: drupalSite.userAgent,
    });
    console.log('Language list:', languageListResult);
    ...
    // Step 4: Clear cache to ensure Canvas settings are refreshed
  ...
  test('Language selector is visible', async ({
    page,
    drupal,
    canvas,
  }) => {
    ...
    // Debug: Check the languages API response
    const languagesResponse = await page.evaluate(async () => {
      try {
        const response = await fetch('/canvas-api/languages');
    ...
    console.log('Languages API response:', JSON.stringify(languagesResponse, null, 2));

    // Debug: Check drupalSettings to verify translation settings
    const settings = await page.evaluate(() => {
      return (window as any).drupalSettings?.canvas;
    });
    console.log('Canvas settings:', JSON.stringify(settings, null, 2));
    console.log('devTranslationMode:', settings?.devTranslationMode);
    console.log('contentTranslationEnabled:', settings?.contentTranslationEnabled);
    console.log('configTranslationEnabled:', settings?.configTranslationEnabled);

    // Verify the settings are correct
    expect(settings?.devTranslationMode).toBe(true);
    expect(settings?.contentTranslationEnabled || settings?.configTranslationEnabled).toBe(true);
